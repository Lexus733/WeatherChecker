...
class ApiQuery {
    companion object {
        ...
        const val GET_NOW_DATA = "SELECT * FROM weatherData WHERE date(dt_text) == CURRENT_DATE AND time(dt_text) > CURRENT_TIME group by dt_text order by id DESC LIMIT 10"
        const val GET_FORWARD_DATA = "SELECT * FROM weatherData WHERE date(dt_text) == date(\"now\",:days) group by dt_text"
    }
}